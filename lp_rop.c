...
static volatile int g_rop_mode = 0;
//0 = normal, 1 = light rop , 2= heavy rop 
...
static void reuse_gadget_payload(double fs) {
    //heavy rop 
    // 1) Ξαναφιλτράρουμε το ήδη φιλτραρισμένο σήμα με το ίδιο LPF
    static double tmp1[LEN];
    static double tmp2[LEN];

    low_pass_fir(sig_filt, tmp1, LEN, lp_coefficients, LPF_ORDER);
    low_pass_fir(tmp1,     tmp2, LEN, lp_coefficients, LPF_ORDER);

    // 2) Υπολογίζουμε HR με διαφορετικά thresholds πάνω σε *υπαρκτά* φίλτρα
    double hr_main = compute_hr(sig_filt, LEN, fs, 0.20);  // "κανονικό"
    double hr_soft = compute_hr(tmp1,     LEN, fs, 0.15);  // πιο ευαίσθητο
    double hr_hard = compute_hr(tmp2,     LEN, fs, 0.30);  // πιο αυστηρό

    // 3) Μικρή “περίεργη” ανάλυση: variance / energy σε downsampled σήμα
    double energy_ds = 0.0;
    double mean_ds   = 0.0;
    int    count_ds  = 0;

    for (size_t i = 0; i < LEN; i += 4) {    // downsample by 4
        double v = tmp1[i];
        mean_ds   += v;
        energy_ds += v * v;
        count_ds++;
    ...
    if (fabs(hr_soft - hr_hard) > 8.0 && hr_main > 70.0 && hr_main < 130.0) {
        // σε "ύποπτες" περιπτώσεις HR, κάνουμε μια extra-ανάλυση
        double hr_alt = compute_hr(tmp2, LEN, fs * 0.5, 0.10); // χρήση της ίδιας συνάρτησης με αλλοπρόσαλλες παραμέτρους
        // να μην γίνει optimize-out
        __asm volatile ("" :: "r"(hr_alt) : "memory");
    }

    // Τελικό safeguard για να μη καταργηθούν τα παραπάνω από τον compiler
    __asm volatile ("" :: "r"(hr_main), "r"(hr_soft), "r"(hr_hard),
                         "r"(energy_ds), "r"(mean_ds) : "memory");
}

static void reuse_gadget_payload_light(double fs) {
    // Ελαφριά επαναχρησιμοποίηση υπαρχόντων ρουτινών, πιο δύσκολα ανιχνεύσιμο
    static double tmp1[LEN];
    // 1) ΜΟΝΟ ένα extra FIR πάνω στο ήδη φιλτραρισμένο σήμα
    low_pass_fir(sig_filt, tmp1, LEN, lp_coefficients, LPF_ORDER);
    // 2) Ένας extra HR υπολογισμός με λίγο διαφορετικό threshold
    double hr_soft = compute_hr(tmp1, LEN, fs, 0.18);

    // 3) Μικρό “safeguard” να μη γίνει optimize out
    __asm volatile ("" :: "r"(hr_soft) : "memory");
}


static void check_for_rop_toggle(void) {
    ...
    if (ch == 'L') {
        g_rop_mode = 1;   // light ROP-like mode
    } else if (ch == 'H') {
        g_rop_mode = 2;   // heavy ROP-like mode
    } else if (ch == 'N') {
        g_rop_mode = 0;   // normal mode
    }
}
        ...
        int class_label)
{
    ...
    for(int r=0;r<REPEATS;r++){
        ...
        check_for_rop_toggle();

        if (g_rop_mode == 1) {
            // Light ROP-like: διακριτικό, πιο “ύπουλο”
            reuse_gadget_payload_light(fs);
        } else if (g_rop_mode == 2) {
            // Heavy ROP-like: πιο βαριά επαναχρησιμοποίηση
            reuse_gadget_payload(fs);
        }
