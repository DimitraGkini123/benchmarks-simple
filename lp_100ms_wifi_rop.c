...
// NEW: 5-class labels for your unified model
#define LB_LIGHT_SAFE   0u
#define LB_MEDIUM_SAFE  1u
#define LB_HEAVY_SAFE   2u
#define LB_LIGHT_ROP    3u
#define LB_HEAVY_ROP    4u
...
// ===================== ROP simulation buffer =====================
#define RW_BUF_N 4096u
...
// level: 0=none, 1=light_rop, 2=heavy_rop
static inline void simulated_rop_payload(uint32_t level, double fs) {
    ...
    for (uint32_t i = 0; i < RW_BUF_N; i += step) {
        ...
        acc ^= rw_buf[i];
        if ((i & 255u) == 0u) net_service();
    }

    if (level >= 2) {
        // heavier: 1 more pass + more memory touch
...
static inline void run_workload_step(uint32_t label) {
    const double fs = 250.0;

    uint32_t workload = 0;   // 0 light, 1 medium, 2 heavy
    uint32_t rop_level = 0;  // 0 none, 1 light_rop, 2 heavy_rop

    switch (label) {
        case LB_LIGHT_SAFE:  workload = 0; rop_level = 0; break;
        case LB_MEDIUM_SAFE: workload = 1; rop_level = 0; break;
        case LB_HEAVY_SAFE:  workload = 2; rop_level = 0; break;
        case LB_LIGHT_ROP:   workload = 0; rop_level = 1; break;
        case LB_HEAVY_ROP:   workload = 2; rop_level = 2; break;
        default:             workload = 1; rop_level = 0; break;
    }

    if (workload == 0) generate_light_signal(fs);
    else if (workload == 1) generate_medium_signal(fs);
    ...
    // >>> ROP-like injection (affects counters, not hash) <<<
    simulated_rop_payload(rop_level, fs);

    hr_sink += compute_hr(sig_filt, LEN, fs, 0.2);

    if (workload == 0) sleep_ms(2);
}
...
static void handle_line(char *line) {
    ...
    if (strstr(line, "\"type\":\"GET_WINDOWS\"")) {
        ...
        for (int i = 0; i < snap_n; i++) {
            ...
            // keep it minimal (as you had) — adjust if your model needs more features
           pos += snprintf(out + pos, (size_t)(out_sz - pos),
                    "{\"window_id\":%u,\"label\":%u,"
...
int main(void) {
    ...
    for (uint32_t phase = 0; phase < 5; phase++) {
        ...
        while (1) {
            net_service();
            run_workload_step(current_label);
