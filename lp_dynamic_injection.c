...
// -----------------------------------------------------------
// DYNAMIC CODE INJECTION – εκτελείται μέσα στο workload
// -----------------------------------------------------------
...
static void malicious_micro_payload(void) {
    ...
    // Chaotic memory access pattern → LSUCNT / timing
    for (int r = 0; r < 3; ++r) {
    ...
    // Branch-heavy arithmetic → CPICNT / FOLDCNT
    int acc = 0;
...
int main(void)
{
    ...
    printf("# FIRMWARE VARIANT: DYNAMIC_INJECT\n");
    printf("\n===== LIGHT WORKLOADS =====\n\n");
